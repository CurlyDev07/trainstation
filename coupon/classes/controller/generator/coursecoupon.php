    ...
    protected function process_page_4() {
        ...
        if ($mform->is_cancelled()) {
            ...
            if ($generatoroptions->generatormethod == 'csv') {
                ...
                $generatoroptions->senddate = strtotime("+2 minutes", time());

                // echo '<pre>';
                // var_dump([
                //     'coupon generated' => $data->date_send_coupons,
                //     'coupon generated_date' => date("F d, Y h:i:s A", $data->date_send_coupons),
                //     'manual' => time(),
                //     'manual_date' => date("F d, Y h:i:s A", time()),
                //     'additional minute' => [
                //         "add 1min" => strtotime("+2 minutes", time()),
                //         'add 1min string' => date("F d, Y h:i:s A", strtotime("+2 minutes", time()))
                //     ]
                // ]);
                // die();
